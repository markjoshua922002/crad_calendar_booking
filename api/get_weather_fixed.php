...
$city = isset($_GET['city']) ? $_GET['city'] : 'Manila';
...
try {
    ...
    if (function_exists('curl_init')) {
        error_log("Using cURL to fetch weather data");
        ...
        if ($weatherData === FALSE) {
            ...
            // Don't throw exception yet, try file_get_contents as fallback
            $weatherData = null;
        }
    ...
    // If cURL failed or is not available, try file_get_contents
    if ($weatherData === null) {
    ...
    // Log raw response
    error_log("Raw weather data response: " . $weatherData);
    ...
    if (isset($weatherArray['cod']) && $weatherArray['cod'] != 200) {
        error_log("OpenWeatherMap API error: " . $weatherArray['message']);
        throw new Exception("Weather API error: " . $weatherArray['message']);
    }
